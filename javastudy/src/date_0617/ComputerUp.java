...
public class ComputerUp implements Comparable<ComputerUp>, Comparator<ComputerUp> {
    ...
    private Integer price; //가격(0617)
    private Integer qty; //재고 수량(0617)
    ...
    //스트림을 위한 생성자
    public ComputerUp(String name, String cpu, int ram, String storage, String graphicCard, int price, int qty) {
        this.name=name;
        this.cpu=cpu;
        this.ram=ram;
        this.storage=storage;
    ...
    public boolean equals(Object obj){
        if(obj instanceof ComputerUp target){
            // null만 조심!!
            return Objects.equals(cpu, target.cpu)
                    && Objects.equals(ram, target.ram)
                    && Objects.equals(storage, target.storage)
                    && Objects.equals(graphicCard, target.graphicCard);
        }
        return false;
    }
    @Override
    ...
    public static void main(String[] args){
        ...
        //컴퓨터들 정보
        ComputerUp[] computers = {
                new ComputerUp("A", "i3", 8, "HDD", "GTX1050", 500, 10),
                new ComputerUp("B", "i5", 16, "SSD", "GTX1660", 700, 5),
                new ComputerUp("C", "i7", 32, "SSD", "RTX3060", 1200, 3),
                new ComputerUp("D", "i7", 64, "NVMe", "RTX4090", 2500, 2),
                new ComputerUp("E", "i3", 4, "HDD", "None", 400, 8)
        };

        //재고 총 금액
        Stream<ComputerUp> intStream = Arrays.stream(computers);
        intStream.forEach(com -> System.out.println("재고 총 금액: " + com.price * com.qty + "원"));
        System.out.println();

        //cpu가 가장 낮은 재고 수량(도움 받음!)
        Stream<ComputerUp> intStream2 = Arrays.stream(computers);
        Stream<ComputerUp> intStream3 = intStream2.filter(item -> item.cpu.equals("i7"));
        int sum = intStream3.mapToInt(item -> item.qty).sum();
        System.out.println("cpu가 가장 낮은 재고의 합: " + sum + "개");
    }
}
