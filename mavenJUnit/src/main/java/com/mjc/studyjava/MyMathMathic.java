...
//        }
    ...
    public Integer[] getIntegerArray(int[] array) {
        if (array == null || array.length == 0) {
            return null; // array가 null이거나 길이가 0인 경우 0 리턴
        }
    ...
    public String makeDecryptString(String encryptedText, int n) {
        if (encryptedText == null || n < 5) {
            return "";
        }

        StringBuilder result = new StringBuilder();

        for (int i = n - 1; i < encryptedText.length(); i += n) {
    ...
    public static void main(String[] args){
        ...
        System.out.println("text 문자열의 각 한 문자를 n 번째 배수 인덱스에 배치하는 랜덤 문자열을 리턴: " + math.makeDecryptString("mjc813", 5));
        System.out.println("code 문자열에서 n번째 배수 인덱스 문자를 뽑아서 하나의 문자열로 리턴: " + math.makeDecryptString("a2B3Cm9Xpjg8KQs1D3mjE1", 5));
    }
}
